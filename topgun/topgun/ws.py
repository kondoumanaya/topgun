...
class WebSocketApp:
    ...
    def url(self) -> str:
        ...
        WebSocket の接続 URL  。
        接続中に値を変更した場合は、再接続時に設定 URL に接続される。
        """
        return self._url
    ...
    def current_ws(self) -> aiohttp.ClientWebSocketResponse | None:
        ...
        現在の WebSocket コネクション 。
        現在のコネクションが存在する場合は ClientWebSocketResponse を返する。
        コネクションが存在しない場合は None を返する。
        """
    ...
    async def heartbeat(self, timeout: float = 10.0) -> None:
        ...
        WebSocket の Ping-Pong で接続の疎通を確認する。
        ユーザーコード内で WebSocket のデータを利用する前にこのコルーチンを待機することで、
        WebSocket の接続性を保証するのに役に立つ。

        一定時間 Pong が返ってこない場合は再接続を試む。
        このメソッドは疎通が確認されるまで待機する。
    ...
    async def wait(self) -> None:
        ...
        WebSocketApp の待機を待つ。
        ただし WebSocketApp は自動再接続の機構を備えるので、実質的に無限待機 。
        プログラムの終了を防ぐのに役に立つ。
        """
        await self._task
