...
class DataStore:
    ...
    def get(self, item: Item) -> Item | None:
        """DataStore から Item を取得する。

        Args:
            item: DataStore のキー (:attr:`.DataStore._KEYS`) を指定する辞書

        Returns:
            キーに一致するアイテムがあればそのアイテムを返する。
            なければ None を返する
        """
        if self._keys:
            try:
    ...
    def find(self, query: Item | None = None) -> list[Item]:
        """DataStore から Item のリストを取得する。

        Args:
            query: DataStore をフィルタするクエリ辞書

        Returns:
            クエリの指定がなければ全件データを返する。
            クエリの指定があれば、それに一致するデータを返する
        """
        if query:
            return [
                item
...
class StoreChange:
    ...
    Attributes:
        ...
        operation: 変更オペレーション
        source: 変更に影響したデータ。 なければ None が格納される
        data: 変更されたデータ
...
class DataStoreCollection:
    ...
    DataStore のコレクションクラス 。
    DataStore を作成することができ、 1 つまたは複数の DataStore を管理する。
    """
    ...
    def onmessage(self, msg: Any, ws: ClientWebSocketResponse | None = None) -> None:
        ...
        :meth:`.Client.ws_connect` に渡すコールバック 。
    ...
    async def wait(self) -> None:
        """DataStoreCollection の onmessage ハンドラが呼び出しされるまで待機する。"""
        event = asyncio.Event()
        self._events.append(event)
        await event.wait()
