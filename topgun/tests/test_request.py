...
import topgun.auth
from topgun.request import ClientRequest, ContentTypeHosts
...
async def test_request_without_auth(mocker: pytest_mock.MockerFixture):
    ...
    items = {
        "example.com": topgun.auth.Item("example", m_auth),
    }
    mocker.patch.object(topgun.auth.Hosts, "items", items)
...
async def test_request_with_selector(mocker: pytest_mock.MockerFixture):
    ...
    items = {
        "example.com": topgun.auth.Item(m_selector, m_auth),
    }
    mocker.patch.object(topgun.auth.Hosts, "items", items)
    m_sesison = mocker.MagicMock()
