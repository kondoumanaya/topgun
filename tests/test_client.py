...
async def test_client():
    ...
    async with topgun.Client(apis=apis, base_url=base_url) as client:
    ...
    assert "topgun" in client._session.headers["User-Agent"].split("/")[0]
    assert client._session.headers["User-Agent"].split("/")[1] == topgun.__version__
...
async def test_client_warn(mocker: pytest_mock.MockerFixture):
    ...
    base_url = "http://example.com"
    async with topgun.Client(apis=apis, base_url=base_url) as client:  # type: ignore
        assert isinstance(client._session, aiohttp.ClientSession)
        assert not client._session.closed
    assert client._base_url == base_url
    assert client._session.closed
...
async def test_client_base_url() -> None:
    ...
    async with (
        ...
        async with topgun.Client(base_url=base_url) as client:
...
def test_client_load_apis_current(mocker: pytest_mock.MockerFixture):
    ...
    assert topgun.Client._load_apis(None) == {"foo": "bar"}
...
def test_client_load_apis_invalid(mocker: pytest_mock.MockerFixture):
    ...
    assert topgun.Client._load_apis(invalid_apis) == {}
...
def test_client_request_get(
    ...
    topgun_client.request("GET", "http://example.com", params={"foo": "bar"})
...
def test_client_request_post(
    ...
    topgun_client.request("POST", "http://example.com", data={"foo": "bar"})
...
async def test_client_fetch(mocker: pytest_mock.MockerFixture):
    ...
    m_req = mocker.patch("topgun.client.Client.request")
    m_req.return_value = m_actx

    async with topgun.Client() as client:
        r = await client.fetch("GET", "http://example.com", params={"foo": "bar"})

    assert isinstance(r, topgun.FetchResult)
...
async def test_client_fetch_error(mocker: pytest_mock.MockerFixture):
    ...
    assert isinstance(r.response, type(m_resp))
    assert r.text == m_resp.text.return_value
    assert isinstance(r.data, topgun.NotJSONContent)
    assert not r.data
    assert m_req.called
...
async def test_client_ws_connect(mocker: pytest_mock.MockerFixture):
    m = mocker.patch("topgun.client.WebSocketApp", new_callable=AsyncMock)
