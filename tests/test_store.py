...
def test_dsm_construct():
    dsm = topgun.store.DataStoreCollection()
    dsm._create("example")
    ...
    assert isinstance(dsm["example"], topgun.store.DataStore)
    assert isinstance(
        dsm._get("example", topgun.store.DataStore), topgun.store.DataStore
    )
...
def test_dsm_subcls_construct():
    ...
    class SubDataStoreCollection(topgun.store.DataStoreCollection):
        def _init(self):
            nonlocal called
            called = True
...
async def test_dsc_onmessage(echo_client: TestClient) -> None:
    class DSC(topgun.store.DataStoreCollection):
...
def test_dsc_onmessage_without_ws(capsys: pytest.CaptureFixture) -> None:
    dsc = topgun.store.DataStoreCollection()
...
def test_ds_construct():
    ...
    class DataStoreWithKeys(topgun.store.DataStore):
        _KEYS = ["foo", "bar"]
...
def test_ds_subcls_construct():
    ...
    class SubDataStore(topgun.store.DataStore):
        def _init(self):
            nonlocal called
            called = True
...
def test_ds_sweep_without_key():
    data = [{"foo": f"bar{i}"} for i in range(1000)]
    ds = topgun.store.DataStore(data=data)
    ds._MAXLEN = len(data) - 100
...
def test_ds_insert():
    ...
    ds1 = topgun.store.DataStore(keys=["foo"])
    ds1._insert(data)
    ...
    ds3 = topgun.store.DataStore(keys=["invalid"])
    ds3._insert(data)
    assert len(ds3._data) == 0
    assert len(ds3._index) == 0

    ds4 = topgun.store.DataStore(keys=["foo"], data=[{"foo": "bar1", "old": "baz"}])
...
def test_ds_update():
    ...
    ds1 = topgun.store.DataStore(keys=["foo"], data=data)
    ds1._update(data)
    assert len(ds1._data) == 1000
    assert len(ds1._index) == 1000

    ds2 = topgun.store.DataStore(keys=["foo"], data=data)
    ds2._update(newdata)
    ...
    ds4 = topgun.store.DataStore(keys=["invalid"])
    ds4._update(data)
...
def test_ds_delete():
    ...
    ds3 = topgun.store.DataStore(keys=["foo"], data=data)
    ds3._delete(invalid)
    assert len(ds3._data) == 1000
...
def test_ds_remove():
    ...
    ds1 = topgun.store.DataStore(keys=["id"], data=data)
    assert len(ds1._data) == 1000
    assert len(ds1._index) == 1000
    ...
    ds2 = topgun.store.DataStore(data=data)
    assert len(ds2._data) == 1000
    assert len(ds2._index) == 0
...
def test_ds_pop():
    ...
    ds3 = topgun.store.DataStore(keys=["foobar"], data=data)
...
def test_ds_find_with_uuid():
    ds = topgun.store.DataStore(data=[{"id": 1}, {"id": 2}])
...
def test_ds_sorted(test_input, query, limit, expected):
    ds = topgun.store.DataStore(keys=["name", "id", "sort_type"], data=test_input)
    actual = ds._sorted(
        item_key="sort_type",
        item_asc_key="asc",
        item_desc_key="desc",
...
async def test_ds_wait_functional():
    ds = topgun.store.DataStore(keys=["id"])
    loop = asyncio.get_running_loop()
...
def test_ds_put():
    ...
    assert result == topgun.store.StoreChange(ds, operation, source, item)


def test_ds_watch():
    ds = topgun.store.DataStore()

    assert isinstance(ds.watch(), topgun.store.StoreStream)
...
async def test_store_stream():
    ...
    with ds.watch() as stream:
        assert isinstance(stream, topgun.store.StoreStream)
        ...
        async def _inner():
            async for change in stream:
                ...
                assert isinstance(change.store, topgun.store.DataStore)
