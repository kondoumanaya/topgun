...
async def test_websocketapp_ensure_open_hdlr(
    ...
    records = [x for x in caplog.records if x.name == "topgun.ws"]
    assert records == []  # No handler errors


def test_heartbeathosts():
    assert hasattr(topgun.ws.HeartbeatHosts, "items")
    assert isinstance(topgun.ws.HeartbeatHosts.items, dict)
    for host, func in topgun.ws.HeartbeatHosts.items.items():
...
async def test_wsresponse_heartbeat(
    ...
    mocker.patch.object(topgun.ws.HeartbeatHosts, "items", items)
    m_resp = MagicMock()
    m_resp.url = test_input
    m_resp.__dict__["_auth"] = None

    wsresp = topgun.ws.ClientWebSocketResponse(
        reader=AsyncMock(),
        writer=AsyncMock(),
        protocol=None,
        response=m_resp,
...
def test_authhosts():
    assert hasattr(topgun.ws.AuthHosts, "items")
    assert isinstance(topgun.ws.AuthHosts.items, dict)
    for host, item in topgun.ws.AuthHosts.items.items():
        assert isinstance(host, str)
        assert isinstance(item, topgun.ws.Item)
...
async def test_wsresponse_auth(mocker: pytest_mock.MockerFixture, test_input, expected):
    m_auth = AsyncMock()
    items = {
        "example.com": topgun.ws.Item("example", m_auth),
    }
    mocker.patch.object(topgun.ws.AuthHosts, "items", items)
    m_resp = MagicMock()
...
async def test_wsresponse_send_str(
    ...
    mocker.patch.object(topgun.ws.RequestLimitHosts, "items", items)
    m_resp = MagicMock()
    m_resp.__dict__["_auth"] = None
    m_resp.url = test_input
...
@pytest.mark.parametrize(
    ...
    [
        (
            ...
            {
                "argument": call({"foo": "bar"}, auth=topgun.auth.Auth),
                "url": URL("ws://example.com"),
                "apis": {"example": ["KEY", b"SECRET"]},
            },
            {
        ...
        (
            ...
            {
                "argument": call(data={"foo": "bar"}, auth=topgun.auth.Auth),
                "url": URL("ws://example.com"),
                "apis": {"example": ["KEY", b"SECRET"]},
            },
            {
        ...
        (
            ...
            {
                "argument": call(None, auth=topgun.auth.Auth),
                "url": URL("ws://example.com"),
                "apis": {"example": ["KEY", b"SECRET"]},
            },
            {
...
async def test_wsresponse_send_json_msgsign(
    ...
    m_msgsign = MagicMock()
    items = {
        "example.com": topgun.ws.Item("example", m_msgsign),
    }
    mocker.patch.object(topgun.ws.MessageSignHosts, "items", items)
    m_resp = MagicMock()
    m_resp.__dict__["_auth"] = None
    m_resp.url = test_input["url"]
    m_resp._session.__dict__["_apis"] = test_input["apis"]
...
@pytest.mark.parametrize(
    ("test_input",),
    [
        (topgun.ws.Heartbeat.bybit,),
        (topgun.ws.Heartbeat.bitbank,),
        (topgun.ws.Heartbeat.phemex,),
        (topgun.ws.Heartbeat.okx,),
        (topgun.ws.Heartbeat.bitget,),
        (topgun.ws.Heartbeat.mexc,),
        (topgun.ws.Heartbeat.kucoin,),
        (topgun.ws.Heartbeat.okj,),
        (topgun.ws.Heartbeat.hyperliquid,),
    ],
)
...
async def test_heartbeat_bittrade(mocker: pytest_mock.MockerFixture, test_input):
    ...
    await asyncio.wait_for(topgun.ws.Heartbeat.bittrade(m_wsresp), timeout=5.0)
...
async def test_auth_bybit_ws(
    ...
    await asyncio.wait_for(topgun.ws.Auth.bybit(m_wsresp), timeout=5.0)
...
async def test_auth_bitflyer_ws(
    ...
    await asyncio.wait_for(topgun.ws.Auth.bitflyer(m_wsresp), timeout=5.0)
...
async def test_auth_phemex_ws(
    ...
    await asyncio.wait_for(topgun.ws.Auth.phemex(m_wsresp), timeout=5.0)
...
async def test_auth_okx_ws(
    ...
    await asyncio.wait_for(topgun.ws.Auth.okx(m_wsresp), timeout=5.0)
...
async def test_auth_bitget_ws(
    ...
    await asyncio.wait_for(topgun.ws.Auth.bitget(m_wsresp), timeout=5.0)
...
async def test_auth_mexc_ws(mocker: pytest_mock.MockerFixture):
    ...
    await topgun.ws.Auth.mexc(m_wsresp)
...
async def test_auth_okj_ws(
    ...
    await asyncio.wait_for(topgun.ws.Auth.okj(m_wsresp), timeout=5.0)
...
async def test_auth_bittrade_ws(test_input, expected, caplog):
    ...
    await asyncio.wait_for(topgun.ws.Auth.bittrade(m_wsresp), timeout=5.0)
...
async def test_auth_coincheck_ws(test_input, expected, caplog) -> None:
    ...
    with freezegun.freeze_time(datetime(2036, 2, 6, 6, 28, 16, tzinfo=timezone.utc)):
        await asyncio.wait_for(topgun.ws.Auth.coincheck(m_wsresp), timeout=5.0)
...
async def test_ratelimit_gmocoin(mocker: pytest_mock.MockerFixture):
    ...
    await asyncio.wait_for(
        topgun.ws.RequestLimit.gmocoin(m_wsresp, m_send_str), timeout=5.0
    )
...
async def test_ratelimit_binance(mocker: pytest_mock.MockerFixture):
    ...
    await asyncio.wait_for(
        topgun.ws.RequestLimit.binance(m_wsresp, m_send_str), timeout=5.0
    )
...
def test_msgsign_binance(mocker: pytest_mock.MockerFixture, test_input, expected):
    ...
    topgun.ws.MessageSign.binance(m_wsresp, test_input["data"])
...
def test_msgsign_bybit(mocker: pytest_mock.MockerFixture, test_input, expected):
    ...
    topgun.ws.MessageSign.bybit(m_wsresp, test_input["data"])
