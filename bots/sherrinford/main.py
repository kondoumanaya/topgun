...
def load_environment():
    ...
    env_files = [
        ROOT_DIR / '.env',                    # 1. 基本設定
        ROOT_DIR / f'config/{os.getenv("ENVIRONMENT", "development")}.env',  # 2. 環境固有
...
class SherrinfordBot:
    ...
    async def create_order(self, symbol: str, side: str, quantity: float, price: float) -> Optional[dict]:
        """注文作成"""
        try:
            ...
            signature = sign_typed_data(
                self.config.private_key, domain, types, message
            )
    ...
    def _risk_check(self, symbol: str, quantity: float) -> bool:
        ...
        try:
            ...
            if self.error_count > 10:
                self.logger.warning(f"⚠️ エラー頻発により取引停止: {self.error_count} errors")
                return False
            ...
            while self.is_running:
                try:
                    ...
                    if ENVIRONMENT == "development" and loop_count % 60 == 0:
                        ...
                        self.logger.info(f"📊 統計: 注文{self.order_count}回, エラー{self.error_count}回")
